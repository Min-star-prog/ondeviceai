...
module data_mem (
    ...
    always_comb begin
        drdata = 32'd0;
        case (mem_mode)
            `LW: drdata = data_ram[daddr[31:2]];      //조합으로 내보내야 single cycle로 처리된다.
            `LH: if(daddr[1])   drdata = {{16{data_ram[daddr[31:2]][31]}},data_ram[daddr[31:2]][31:16]};      //조합으로 내보내야 single cycle로 처리된다.
            ...
            `LHU: if(daddr[1])   drdata[15:0] = data_ram[daddr[31:2]][31:16];      //조합으로 내보내야 single cycle로 처리된다.
                 else           drdata[15:0] = data_ram[daddr[31:2]][15:0] ;

                
            `LBU: if(daddr[1:0] == 2'b11) drdata[7:0] = data_ram[daddr[31:2]][31:24] ;  
                else if(daddr[1:0] == 2'b10) drdata[7:0] = data_ram[daddr[31:2]][23:16] ;       //daddr이 1씩 증가 
                else if(daddr[1:0] == 2'b01) drdata[7:0] = data_ram[daddr[31:2]][15:8]  ;       //daddr이 1씩 증가
                else drdata[7:0] = data_ram[daddr[31:2]][7:0]; 
