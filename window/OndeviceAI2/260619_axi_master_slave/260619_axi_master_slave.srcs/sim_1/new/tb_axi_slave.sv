...
module tb_axi_slave ();
    ...
    myip_v1_0 #(C_S00_AXI_DATA_WIDTH, C_S00_AXI_ADDR_WIDTH) dut (.*);


    always #5 s00_axi_aclk = ~s00_axi_aclk;


// valid 나가고 ready 받고 resp 나가고~valid 나가고 ready 받고 resp 나가고~
    task automatic axi_write(input logic [31:0] wr_addr, input logic [31:0] wr_data);
        begin
            @(posedge s00_axi_aclk);

            s00_axi_awaddr  <= wr_addr;
            ...
            @(posedge s00_axi_aclk);
            s00_axi_bready <= 1'b0;
            $display("[%t] WRITE : addr = 0x%0h, wdata = 0x%08h", $time, wr_addr, wr_data);
        end
    endtask
    ...
    task automatic axi_read(input logic [31:0] addr);
        begin
            ...
            wait (s00_axi_rvalid);
            @(posedge s00_axi_aclk);
            rdata_reg <= s00_axi_rdata;
            s00_axi_rready <= 1'b0;
            @(posedge s00_axi_aclk);

            $display("[%t] READ : addr = 0x%0h, rdata = 0x%08h", $time,addr, rdata_reg);
        end
    endtask
    ...
    initial begin
        ...
        axi_write(32'h0000_0008, 32'h12345678);
        axi_write(32'h0000_000C, 32'hAAAABBBB);
