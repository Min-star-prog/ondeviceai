...
class driver;
    ...
    task preset();
        ...
        assert (!fifo_vif.full) $display("[DRV Assert] reset pass : NOT full!");
...
class monitor;
    ...
    task run();
        forever begin
            @(negedge fifo_vif.clk);  //예전에 negedge 왜 안 됐지?
            tr           = new;
        ...
        forever begin
            ...
            if (tr.pop && (!tr.empty)) begin
                ...
                if (tr.pop_data == compare_data) begin
                    $display("%t : PASS !! pop = %d, pop_data = %d ", $time, tr.pop, tr.pop_data);
                    pass_cnt++;
                end else begin
